...
def seg_lin_reg(x, y, tol=0.5, too_big=5, variance=None):
    # if |ynew - ypredicted| > tol*std, new a new segment is started
    # return list of (slice, alpha, beta)
    assert len(x) == len(y)
    assert len(x) > 0

    std_y_global = math.sqrt(np.var(y)) if not variance else variance
    ...
    while i < len(x):
        ...
        n = float(i - i0 + 1 - ignored)
        if n == 1.:
            raise ValueError("i={}, i0={},ignored={}".format(i, i0, ignored))
        x_new = x[i]
        y_new = y[i]
        ...
        if np.isnan(beta):
            raise ValueError('beta is nan. beta0={}, n={}, dx={}, dy={}, var0={}, var={}'.format(beta0, n, dx, dy, var0, var))
        if np.isnan(alpha):
        ...
        # show()
        #if i - i0 > 1 and abs(y_new - y_exp) > var_y_global * too_big:
        #    ignored += 1
        #    pass
        if i - i0 > 1 and abs(y_new - y_exp) > tol * std_y_global:
            print('hey ' + str(i))
            result += [(slice(i0, i, 1), alpha0, beta0)]
            i0 = i - 1
            ignored = 0
            mean_x0 = x[i0]
            mean_y0 = (y[i0] + y_new)/2.
            var0 = 0.
            beta0 = 0.
            alpha0 = mean_y0
            continue
...
def seg_lin_reg2(x, y, tol=0.5, too_big=5, min_len=4, expected_variance=None):
    # if |ynew - ypredicted| > tol*std, new a new segment is started
    # return list of (slice, alpha, beta)
    assert len(x) == len(y)
    assert len(x) > 0

    var_y_global = math.sqrt(np.var(y)) if not expected_variance else expected_variance
...
def _test5(tol):
    np.random.seed(5)
    n = 100
    peaks = np.random.randint(0, n, 0)
    y = np.random.normal(0, 0.1, n)
    for p in peaks:
        y[p] = 20
    x = np.arange(n)

    plot_segments(x, y, tol)


def plot_segments(*args, **kwargs):
    segs = seg_lin_reg(*args, **kwargs)
    x = args[0]
    y = args[1]
